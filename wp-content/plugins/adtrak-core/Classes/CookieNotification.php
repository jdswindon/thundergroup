...
class CookieNotification
{
	...
	public function create_page()
	{
		...
		if ( ! empty( $PPquery->post ) ) {
			$privacyPage = $PPquery->post;
		} else {
			$privacyPage = null;
		}

		if ($privacyPage == null && $cookiePage == null) {
			$status = 'publish';
		} elseif($cookiePage == null) {
			$status = 'draft';
		}
